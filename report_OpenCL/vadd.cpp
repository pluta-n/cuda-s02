...
int main(void)
{
    ...
    cl::Buffer d_e;                        // device memory used for the input  b vector
    cl::Buffer d_f;                       // device memory used for the output c vector
    cl::Buffer d_g;                        // device memory used for the input  a vector
    ...
    try 
    {
        ...
        for(int i = 0; i < count; i++) {
            ...
            if(tmp*tmp < TOL*TOL) {      // correct if square deviation is less 
                ...
                printf(
                    " tmp %f h_a %f h_b %f  h_c %f h_d %f h_e %f  h_f %f h_g %f\n",
                    tmp, 
                    h_a[i], 
                    h_b[i], 
                    h_c[i],
